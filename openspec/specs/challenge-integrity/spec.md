...
## Purpose
Keep the CTF-8 challenge internally consistent and reproducible: its documented walkthrough, build inputs, container entrypoint, and credentials stay in sync so a player-facing instance boots, every documented stage succeeds against the files as built, and no flag path, credential, or dependency silently drifts.
